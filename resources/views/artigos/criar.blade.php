...
@section('content')
    <form action="{{ route('artigos.store') }}" method="post">
        ...
        <input class="form-control" type="text" id="title" name="title" value="{{ $artigo->title ?? '' }}">
        <label class="form-label" for="body">Mensagem</label>
        <textarea class="form-control" type="text" id="body" name="body" cols="5">{{ $artigo->body ?? '' }}</textarea>
        <div class="row">
            <div class="col">
